...
class JWT{
    ...
    public static function create(object $data, $secret_key, $expire = 3600){
        $header = json_encode(['alg' => 'HS256', 'typ' => 'JWT']);
        $time = time();
        $values = array(
            'jti' => base64_encode(openssl_random_pseudo_bytes(32)), // ID único del token
            'iat' => $time,  // Tiempo de emisión
            'exp' => $time + $expire, // Tiempo de expiración
            'sub' => $data->id,  // ID del usuario como claim "sub"
            'data' => $data   // Otros datos del usuario
        );
